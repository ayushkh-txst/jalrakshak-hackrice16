...
type Message = { id: string; role: 'assistant' | 'user'; text: string; createdAt: number; actions?: NavCatAction[] };
...
function speak(text:string) { if (!voiceEnabled || !('speechSynthesis' in window)) return; window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(text); utterance.rate = crisisMode ? 0.92 : 1; window.speechSynthesis.speak(utterance); }
...
function renderMessages() { const visible = messages.length ? messages : [createMessage('assistant', WELCOME_TEXT)]; return visible.map((message) => `<div class="ai-message ${message.role}" data-message-id="${escapeHtml(message.id)}"><div class="ai-message-wrap"><div class="ai-bubble"><div>${escapeHtml(message.text)}</div>${message.actions?.length ? `<div class="navcat-result-actions">${message.actions.map((action,index) => renderAction(action,message.id,index)).join('')}</div>` : ''}</div>${messages.length ? `<button type="button" class="ai-delete-message" data-ai-delete="${escapeHtml(message.id)}" aria-label="Delete this message">×</button>` : ''}</div></div>`).join(''); }
  ...
  section.innerHTML = `<div class="navcat-shell">
    ...
    <main class="navcat-chat-main">
      ...
      <div class="ai-layout"><section class="ai-chat-card">
        ...
        <div class="ai-disclaimer">NavCat can explain JalRakshak and guide you through it. GPS, routing, hazard screening, emergency records, and hotline data come from validated systems.</div>
      </section></div>
    </main>
  </div>`;
...
async function addUserMessage(section:HTMLElement, text:string) {
  ...
  composerDraft = '';
  messages.push(createMessage('user',clean)); saveHistory(); responsePending = true; renderAssistant(section);
...
function startVoice(section:HTMLElement) { const browser = window as any; const Recognition = (browser.SpeechRecognition || browser.webkitSpeechRecognition) as SpeechRecognitionCtor | undefined; if (!Recognition) { messages.push(createMessage('assistant','Voice input is not supported in this browser. You can still type a short request.')); saveHistory(); renderAssistant(section); return; } const recognition = new Recognition(); recognition.lang='en-US'; recognition.interimResults=false; recognition.continuous=false; listening=true; renderAssistant(section); recognition.onresult=(event:any)=>{ listening=false; void addUserMessage(section,event.results?.[0]?.[0]?.transcript ?? ''); }; recognition.onerror=()=>{ listening=false; renderAssistant(section); }; recognition.onend=()=>{ if(listening){ listening=false; renderAssistant(section); } }; recognition.start(); }
function wireAssistant(section:HTMLElement) {
  const input = section.querySelector<HTMLInputElement>('[data-ai-input]');
  input?.addEventListener('input', () => { composerDraft = input.value; });
  section.querySelector<HTMLFormElement>('.ai-input-row')?.addEventListener('submit', (event) => { event.preventDefault(); if (input) void addUserMessage(section,input.value); });
